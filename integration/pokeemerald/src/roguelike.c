...
void Rogue_BeginRunFromHub(const RogueStartConfig *config, RogueRunState *state, RogueHubState *hub)
{
    ...
    state->totalFloors = config->totalFloors;
    state->currentFloor = 0;

    if (hub->unlockedTeamSize < config->startingTeamSize)
        hub->unlockedTeamSize = config->startingTeamSize;

    hub->upgradeLevel += config->levelBonus;
...
{
    outData->seed = state->seed;
    outData->totalFloors = state->totalFloors;
    outData->currentFloor = state->currentFloor;
    outData->hubCurrency = hub->hubCurrency;
    outData->hubUpgradeLevel = hub->upgradeLevel;
    outData->hubUnlockedTeamSize = hub->unlockedTeamSize;
    outData->hubMedals = hub->medals;
}

void Rogue_LoadFromData(const RogueSaveData *data, RogueRunState *outState, RogueHubState *outHub)
{
    outState->seed = data->seed;
    outState->totalFloors = data->totalFloors;
    outState->currentFloor = data->currentFloor;

    outHub->hubCurrency = data->hubCurrency;
    outHub->upgradeLevel = data->hubUpgradeLevel;
    outHub->unlockedTeamSize = data->hubUnlockedTeamSize;
    outHub->medals = data->hubMedals;
}
