...
- `integration/pokeemerald/src/roguelike_runtime.c`: capa runtime para loop de piso/nodo/save más fácil de conectar en engine.
...
2. Opción rápida: usar `roguelike_runtime.h` (`RogueRuntime_BeginNewRun`, `RogueRuntime_BuildCurrentFloor`, `RogueRuntime_AutoPickNode`, `RogueRuntime_PlayStep`, `RogueRuntime_Save`).
3. Opción granular: `Rogue_InitRun(...)` y `Rogue_GenerateFloor(...)` para construir el mapa de cada run.
4. Llamar `Rogue_GenerateEnemyTeam(...)` para crear equipo enemigo escalado al nivel/cantidad del equipo del jugador.
5. Llamar `Rogue_BuildBattlePlan(...)` para resolver IA/recompensa/tipo de encuentro por nodo (wild/trainer/elite/boss/support/event).
6. Llamar `Rogue_ResolveNode(...)` para obtener recompensa/efecto del nodo elegido (oro, heal/shop).
7. Gestionar el Hub con `Rogue_HubInit`, `Rogue_HubApplyResolution`, `Rogue_HubUpgradeTeamSize`, `Rogue_HubUpgradePower` (formación/mejora de equipo entre runs).
8. Evaluar progreso de medallas con `Rogue_EvaluateGymLeader(...)` cuando toque líder difícil.
9. Llamar `Rogue_AdvanceRun(...)` y comprobar `Rogue_RunIsFinished(...)` para cerrar run al boss final.
10. Arrancar run desde hub con `Rogue_BeginRunFromHub(...)` (seed/floors/config inicial de equipo).
11. Guardar y cargar con `Rogue_SaveToData(...)` / `Rogue_LoadFromData(...)` para persistencia tipo saveblock.
12. Guardar `RogueRunState` y estado de hub en saveblock para retomar la run.
13. Cada nodo ya trae `mapGroup/mapNum` y puedes usar `Rogue_GenerateZoneChoices(...)` para mostrar 2-3 destinos al cambiar de zona.
